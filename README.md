...
Vercel serves `dist/public` from the CDN and runs the Express API as a single Node
function at `api/index.js`, generated by `npm run build:vercel`.

Vercel's filesystem is read-only apart from `/tmp`, so the API keeps its SQLite file
at `/tmp/podclub.db`. A cold instance creates the schema from `server/ddl.ts` and
seeds the demo data itself. Everything works, but writes only live as long as that
instance stays warm — fine for a demo, not for real data.

To make it durable, point `SQLITE_PATH` at a mounted volume, or move `server/storage.ts`
onto managed Postgres (Supabase or Neon) by swapping the Drizzle driver. The route
layer and validation do not change.
